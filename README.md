...
- `/pets`：电子宠物（登录后领养，随机分配 5 个系列之一并养成）
...
- **电子宠物（`/pets`）**：登录后可领养宠物，共 **5 个系列**（ST / ESP / Linux / Arm / 传感器），**领养时随机分配**系列；通过喂食 / 玩耍 / 训练加经验升级，可改名 / 放生（每人上限 6 只）。每个系列有**各自的形状**（原创几何造型）。登录后**每个页面左下角都会有浮动宠物挂件**（`public/pets-widget.js`，由 `nav-auth.js` 自动注入；未登录不显示）。接口：`GET /api/pets`、`POST /api/pets`（随机系列）、`POST /api/pets/:id/action`（feed/play/train）、`PATCH /api/pets/:id`（改名）、`DELETE /api/pets/:id`（放生）。表见 [`data/migration-add-pets.sql`](data/migration-add-pets.sql)。
